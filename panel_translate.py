...
def show_panel(root, app):
    panel_width = 118
    panel_height = 480  # во всю высоту окна, как у panel_cfg
    ...
    author = tk.Label(top_bar, text="Translate", bg=DARK_BG, fg="red", font=("Segoe UI", 10, "bold"))
    ...
    make_button(content_frame, "Scan Mods", cmd=app.scan_btn.invoke)
    make_button(content_frame, "Clear", cmd=app.clear_btn.invoke)
    ...
    # --- Остальные Action ---
    make_button(content_frame, "Action 2")
    make_button(content_frame, "Action 3")
    make_button(content_frame, "Action 4")
    make_button(content_frame, "Action 5")

    # --- Back и Exit ---
    back_btn = make_button(content_frame, "Back")
    exit_btn = make_button(content_frame, "Exit", cmd=app.exit_btn.invoke)
